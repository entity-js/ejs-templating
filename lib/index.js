...
templating = module.exports = {
  ...
  registeredEngine: function (name) {
    ...
    return _engines[name] !== undefined;
  },
  ...
  register: function (name, target, weight, engine) {
    ...
    _templates[name].push(tpl);
    sortBy(_templates[name], 'weight');
  ...
  unregister: function (name, target) {
    ...
    if (!target) {
      ...
      var tmp = [];
      for (var i = 0, len = _templates[name].length; i < len; i++) {
        var tpl = _templates[name][i];
  ...
  templates: function (name) {
    ...
    return name ? (_templates[name] ? _templates[name] : []) : _templates;
  },
  ...
  template: function (done, name) {
    ...
    if (tpl.code) {
      ...
      queue.push(function (next) {
        tpl.callback(function (err, code) {
          if (err) {
            return next(err);
          }
  ...
  render: function (done, name, args) {
    ...
    async.series(queue, function (err) {
      ...
      if (_engines[engine] === undefined) {
        return done(new EUnknownEngine(engine));
      }

      _engines[engine].render(done, code, locals);
    });
  }
};
